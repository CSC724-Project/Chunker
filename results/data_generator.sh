...
# BeeGFS Expanded Testing Script
# This script performs extensive testing of BeeGFS with many different combinations
...
if [ ! -f "${CSV_FILE}" ]; then
    echo "file_path,file_size,chunk_size,access_count,avg_read_size,avg_write_size,max_read_size,max_write_size,read_count,write_count,throughput_mbps" > "${CSV_FILE}"
fi

# More varied chunk sizes to test (in KB)
CHUNK_SIZES=(64 128 256 512 768 1024 1536 2048 3072 4096 6144 8192)

# More varied file sizes to test (in MB)
FILE_SIZES=(50 100 200 350 500 750 1000 1500 2000)

# Define different read patterns (number of operations)
READ_PATTERNS=(5 10 15 20 25 30 40 50)

# Define different write patterns (number of operations)
WRITE_PATTERNS=(2 5 8 10 15 20 25 30)

# Create and configure test directories for each chunk size
...
for chunk_size in "${CHUNK_SIZES[@]}"; do
    ...
    if [ ! -d "${dir_name}" ]; then
        ...
        # Set chunk size for this directory
        sudo beegfs-ctl --setpattern --chunksize="${chunk_size}k" "${dir_name}"
        echo "Set chunk size to ${chunk_size}K for ${dir_name}"
    fi
done

# Function to generate a random value in a range
random_range() {
    local min=$1
    local max=$2
...
run_test() {
    local chunk_size=$1     # in KB
    local file_size=$2      # in MB
    local read_count=$3
    local write_count=$4
    ...
    if [ ! -f "${file_path}" ]; then
        ...
        sudo dd if=/dev/zero of="${file_path}" bs=1M count="${file_size}" status=progress
    else
    ...
    # Verify file exists
    if [ ! -f "${file_path}" ]; then
        echo "Failed to create/access file: ${file_path}"
        return 1
    fi
    ...
    for ((i=1; i<=read_count; i++)); do
        # Random block size between 4KB and chunk size (or smaller for small chunk sizes)
        ...
        dd if="${file_path}" of=/dev/null bs=1K skip=$((offset / 1024)) count=${read_size} status=none 2>/dev/null
    ...
    for ((i=1; i<=write_count; i++)); do
        ...
        dd if=/dev/urandom of="/tmp/random_data_$$" bs=1K count=${write_size} status=none 2>/dev/null
        
        # Perform write operation
        dd if="/tmp/random_data_$$" of="${file_path}" bs=1K seek=$((offset / 1024)) count=${write_size} conv=notrunc status=none 2>/dev/null
    ...
    # Measure throughput (read a portion of the file to be quicker)
    echo "Measuring throughput"
    local sample_size=$((file_size > 100 ? 100 : file_size))
    start_time=$(date +%s.%N)
    dd if="${file_path}" of=/dev/null bs=1M count=${sample_size} status=none 2>/dev/null
    end_time=$(date +%s.%N)
    ...
    # Log results to CSV immediately
    echo "${file_path},${file_size}000000,${chunk_size}000,${access_count},${avg_read_size},${avg_write_size},${max_read_size},${max_write_size},${read_count},${write_count},${throughput}" >> "${CSV_FILE}"
    
    echo "Test completed and logged to CSV"
}

# Function to generate random combinations for testing
generate_random_combinations() {
    ...
    for ((i=1; i<=num_combinations; i++)); do
        ...
        # Run the test immediately with this combination
...
# Handle cleanup command if passed
if [ "$1" == "--cleanup" ]; then
