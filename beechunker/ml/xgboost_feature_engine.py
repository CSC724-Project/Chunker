...
class XGBoostFeatureEngine:
    ...
    def _calculate_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["io_per_chunk"] = (df["read_count"] + df["write_count"]) / (
                df["file_size"] / df["chunk_size"]
            )
            df["bytes_per_io"] = df["file_size"] / (
                df["read_count"] + df["write_count"]
            )
            df["throughput_per_chunk"] = df["throughput_mbps"] / (
                df["file_size"] / df["chunk_size"]
            )
            ...
            df["io_efficiency"] = (df["throughput_mbps"] * 1024 * 1024) / (
