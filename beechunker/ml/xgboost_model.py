...
class BeeChunkerXGBoost:
    ...
    def _calculate_io_efficiency(self, df: pd.DataFrame) -> pd.Series:
        ...
        return (df["throughput_mbps"] * 1024 * 1024) / (
            df["file_size"] * (df["read_count"] + df["write_count"])
        )
        ...
        try:
            ...
            if training:
                ...
                if not hasattr(self.scaler, "mean_"):
                    raise RuntimeError(
                        "Scaler is not fitted. Need to train the model first."
                    )
    ...
    def _label_data(self, df: pd.DataFrame) -> tuple:
        ...
        try:
            ...
            total_count = len(df)
            logger.info(
                f"Data labeling complete: {optimal_count}/{total_count} "
        ...
        try:
            ...
            if isinstance(input_data, pd.DataFrame):
                ...
                if len(df) < config.get("ml", "min_training_samples"):
                    logger.warning(
                        "Not enough samples for training. Need at least %d samples.",
            ...
            # Map the column names to what the model expects
            column_mapping = {
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
                ...
                logger.info(
                    f"Fold {fold} - Logloss: {fold_metrics['logloss']:.4f}, "
            ...
            with open(
                os.path.join(self.models_dir, "xgboost_model_info.json"), "w"
            ) as f:
                import json
    ...
    def predict(self, df: pd.DataFrame) -> int:
        """
        Predict optimal chunk size for a single file.
        Note: This will process the first row only if multiple rows are present.
        Returns the optimal chunk size in KB.
        """
        try:
            ...
            # Return optimal chunk size in KB
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            try:
                with open(
                    os.path.join(self.models_dir, "xgboost_model_info.json"), "r"
                ) as f:
    ...
    def set_last_training_time(self):
        ...
        with open(
            os.path.join(self.models_dir, "xgboost_last_training_time.txt"), "w"
        ) as f:
    ...
    def get_last_training_time(self):
        ...
        try:
            with open(
                os.path.join(self.models_dir, "xgboost_last_training_time.txt"), "r"
            ) as f:
