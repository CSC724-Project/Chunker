#!/usr/bin/env python3
import pandas as pd
from beechunker.ml.linear_regression import BeeChunkerLR
...
def train_model(train_csv: str):
    """
    Train the BeeChunkerLR model once and persist it to disk.
    """
    model = BeeChunkerLR()
    success = model.train(train_csv)
    if not success:
        raise RuntimeError(f"LR Training failed on {train_csv}")
    print(f"✅ LR Training completed on {train_csv}")

def predict_optimal_chunk(pred_csv: str) -> int:
    """
    Load the persisted LR model, run predict() on new data,
...
if __name__ == "__main__":
    # === Uncomment to train the LR model (do this only once) ===
